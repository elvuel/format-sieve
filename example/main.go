...
func main() {
	...
	data := `{"title":"tttt","user":{"name":"foo", "Age": 20, "god": "is girl"}}`
	json.Unmarshal([]byte(data), &p1)
	fmt.Printf("%#v\n", p1)

	posts := ss.DefaultAssembler.NewSlice("Post")
	data = `[{"title":"tttt","user":{"name":"foo","Age":20}, "labels":[{"name":"hot", "popular": 30},{"name":"download", "popular": 20}]},{"title":"tttt","user":{"name":"foo","Age":21}}]`
