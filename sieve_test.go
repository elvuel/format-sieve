...
func TestAssembler_Add(t *testing.T) {
	box := NewBox(nil, "Foo")
	box.AddElement(Element{Name: "Field", Tag: "bar", Type: "string"})
	...
	if n := reflect.TypeOf(i).Elem().Field(0).Name; n != "Field" {
		t.Errorf("the field name wants %s, but got %s", "Field", n)
	}
...
func TestBox_AddElement(t *testing.T) {
	...
	eleInvalid := Element{Name: ""}
	box.AddElement(eleInvalid)
	if len(box.Elements) != 2 {
		t.Error("the elements length should be 2")
	}
}
...
func TestBox_Structured(t *testing.T) {
	...
	if !existed {
		t.Error("`baref` filed should be existed")
	}
